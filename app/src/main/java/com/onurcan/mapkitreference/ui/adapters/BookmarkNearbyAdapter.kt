...
class BookmarkNearbyAdapter(
    ...
    RecyclerView.Adapter<BookmarkNearbyAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val cLat = currentItem.poiLat.toDouble()
        val cLong= currentItem.poiLong.toDouble()
        if (mLat >= cLat && mLat <= cLat + 0.02) {
            // mLat is larger than cLat
            holder.parent.visibility = View.VISIBLE
        } else if (mLat <= cLat && mLat >= cLat - 0.02) {
            holder.parent.visibility = View.VISIBLE
        } else {
